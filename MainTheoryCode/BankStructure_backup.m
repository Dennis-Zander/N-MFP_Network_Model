...
for i = 1:10
    ...
    tic
    %counter = 0;
    IB_in  = ones(N_B,1);      % Percentage of IB borrowing to draw from
    IB_out = ones(N_B,1);      % Percentage of IB lending to draw from
    
    while sum(IB_in)>1 && sum(IB_out)>1     % Soft threshold above 0, refine later
        %counter = counter + 1
        % Draw
        b1 = randi(N_B);
        b2 = randi(N_B);
    
        % Replace if self-link
        while b1 == b2
            b2 = randi(N_B);
        end
        ...
        if crit_cond == 1 && A_B(b1,b2,i) == 0
            A_B(b1,b2,i) = link_str;
            IB_in(b1,1)  = IB_in(b1,1)  - link_str;
            IB_out(b1,1) = IB_out(b1,1) - link_str;
        end
    end
    toc
end
